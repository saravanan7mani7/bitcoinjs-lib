...
class Transaction {
  ...
  hashForWitnessV1(inIndex, prevOutScripts, values, hashType, leafHash, annex) {
    ...
    if (!isAnyoneCanPay) {
      ...
      hashPrevouts = Uint8Array.from(bcrypto.sha256(bufferWriter.end()));
      bufferWriter = bufferutils_js_1.BufferWriter.withCapacity(
        8 * this.ins.length,
      );
      values.forEach(value => bufferWriter.writeUInt64(value));
      hashAmounts = Uint8Array.from(bcrypto.sha256(bufferWriter.end()));
      bufferWriter = bufferutils_js_1.BufferWriter.withCapacity(
      ...
      hashScriptPubKeys = Uint8Array.from(bcrypto.sha256(bufferWriter.end()));
      bufferWriter = bufferutils_js_1.BufferWriter.withCapacity(
        4 * this.ins.length,
      );
      this.ins.forEach(txIn => bufferWriter.writeUInt32(txIn.sequence));
      hashSequences = Uint8Array.from(bcrypto.sha256(bufferWriter.end()));
    }
    if (!(isNone || isSingle)) {
      if (!this.outs.length)
      ...
      hashOutputs = Uint8Array.from(bcrypto.sha256(bufferWriter.end()));
    ...
    const payload = new Uint8Array(sigMsg.length + 1);
    payload[0] = 0x00;
